...
function ItemDAO(database) {
    ...
    this.getCategories = function(callback) {
        ...
        */
        ...
        // callback.
        var cursor = this.db.collection('items').aggregate([{$project:{num:1,category:1}},{$group:{_id:"$category",num:{$sum:1}}},{$sort:{_id:1}}]);
    ...
    this.getItems = function(category, page, itemsPerPage, callback) {
        ...
        if(category == 'All'){
          cursor = this.db.collection('items').find({}).skip(skip).limit(itemsPerPage).sort({_id:1});
        }else{
          cursor = this.db.collection('items').find({"category":category}).skip(skip).limit(itemsPerPage).sort({_id:1});
        }
        cursor.each(function(err,doc){
          // console.log(doc);
          if(doc){
    ...
    this.getNumItems = function(category, callback) {
        ...
        //  console.log(category);
         ...
         if(category == "All"){
          //  console.log('hi');
            cursor = this.db.collection('items').find({});
         }else{
            cursor = this.db.collection('items').find({"category":category});
         }
    ...
    this.searchItems = function(query, page, itemsPerPage, callback) {
        ...
        cursor = this.db.collection('items').find({$text:{$search:query}}).skip(skip).limit(itemsPerPage).sort({_id:1});
        cursor.each(function(err,doc){
          if(doc){
            items.push(doc);
          }else{
    ...
    this.getNumSearchItems = function(query, callback) {
        ...
        */
        var cursor = this.db.collection('items').find({$text:{$search:query}});
        cursor.each(function(err,doc){
          if(doc){
            numItems += 1;
          }else{
    ...
    this.getItem = function(itemId, callback) {
        ...
        // to the callback.
        var cursor = this.db.collection('items').find({"_id":itemId});
        cursor.each(function(err,doc){
          if(doc){
            callback(doc);
          }
